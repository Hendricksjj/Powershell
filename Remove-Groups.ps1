...
.EXAMPLE
    Remove-CBCGroups -Username user1
    Removes all groups from user1
.EXAMPLE
    Get-Content -Path 'C:\users.txt' | Remove-CBCGroups
    Finds all users in the file and pipes them to Remove-CBCGroups. Removing all memeberships from each user.
#>
function Remove-CBCGroups {
